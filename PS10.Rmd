...
lambda_inputs <- 10^seq(-2, 0, length = 50)
...
* For model simplicity, I will use the optimal lambda-star within one standard error of the minimal MSE for my predictions. The number of variables used goes from 11 at the optimal lambda-star to just 5 variables when looking at the optimal lambda-star corresponding to the simplest model within 1 SE of the MSE. 


```{r, echo=FALSE}
plot_LASSO_coefficients +
  coord_cartesian(xlim=c(.03, 0.3), ylim=c(-0.015, 0.015)) +
  labs(title = "LASSO Coefficients (red line = lambda star, blue line = lambda star with 1 SE)") +
  theme(legend.position = "bottom")
```

```{r}
library(Metrics)
# Training Score (Root Mean Squared Logarthmic Error)
y_hat <- predict(LASSO_fit, newx=predictor_matrix_train, s=lambda_star_1SE) %>% 
  as.vector()
rmsle(actual = train$price_doc, predicted = exp(y_hat))
...
model_lm_price_doc <- lm(model_formula, data = train)
y_hat <- predict(model_lm_price_doc, newdata= train)
rmsle(actual = train$price_doc, predicted = exp(y_hat))

# Kaggle Score Calculation
y_hat <- predict(model_lm_price_doc, newdata = test)
sample_submission$price_doc <- exp(y_hat)
write.csv(sample_submission, "submission_lm.csv", row.names=FALSE)
...
--------------------- | -------------   | -------------
lm                    | 0.2535                |  2.1700
LASSO crossvalidated  | 0.5701                |  0.4391
